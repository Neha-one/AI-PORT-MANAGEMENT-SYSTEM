...
const usePortStore = create(
  persist(
    (set, get) => ({
      ...
      setBerthForm: (payload) =>
        set({
          berthForm: payload,
        }),

      setContainerForm: (payload) =>
        set({
          containerForm: payload,
        }),
      ...
      loadDashboard: async () => {
        // Don't call protected APIs when user is logged out
        ...
        try {
          ...
          set({
            ...
            authMessage:
              "Backend unavailable, showing polished demo data.",
      ...
      login: async (authForm) => {
        try {
          ...
          // IMPORTANT:
          // Failed login must NOT authenticate the user.
          set({
            admin: null,
          ...
          await get().fetchJson("/api/vessels", {
            ...
            data: {
              ...
              arrivalEta: formState.arrivalEta
                ? new Date(
                    formState.arrivalEta,
                  ).toISOString()
                : null,

              departureEtd: formState.departureEtd
                ? new Date(
                    formState.departureEtd,
                  ).toISOString()
                : null,
          ...
          await get().fetchJson("/api/berths", {
            ...
            data: {
              ...
              capacityLength: Number(
                berthForm.capacityLength,
              ),

              capacityDepth: Number(
                berthForm.capacityDepth,
              ),
        ...
        try {
          await get().fetchJson("/api/containers", {
            ...
            data: {
              ...
              assignedVesselId:
                containerForm.assignedVesselId
                  ? Number(
                      containerForm.assignedVesselId,
                    )
                  : null,
              ...
              assignedYardLocation:
                containerForm.assignedYardLocation,
    ...
    {
      name: "port-management-auth",

      // Only persist authentication information.
      // Password/form/dashboard data will NOT be persisted.
      partialize: (state) => ({
